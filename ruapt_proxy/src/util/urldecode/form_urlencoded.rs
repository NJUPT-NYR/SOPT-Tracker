...
use super::percent_encoding::{percent_decode, percent_encode_byte};
...
impl<'a> Iterator for Parse<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            return Some((decode(name), decode_bin(value)));
        }
    }
}
...
fn decode_bin(input: &[u8]) -> Cow<'_, str> {
    let replaced = replace_plus(input);
